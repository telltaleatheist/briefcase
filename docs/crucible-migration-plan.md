...
**Model pickers as built (2026-09-24).** One source: `GET /crucible/ai/models[?values=…]`, built purely by `crucible/llm/model-options.ts` from the connected server (the best-ranked running one that answers). Its own models are the `analysis` class's candidates (`local_model_choices.analysis`) whose `/v1/models` row is loadable or resident (a server naming no candidates has its non-page-reader text models read instead); each upstream it has *configured* adds what its test lists. Groups follow the server: "On this Crucible" (or "On <server>"), "Claude / OpenAI / Ollama via Crucible" (plus " on <server>" when remote); an unstated size reads "size unknown". Stored values keep Briefcase's `provider:model` spelling (`local:<id>` is a server model) and are resolved with the call-time rules (`ollama:<tag>` → the server's own copy via ollama-map, `anthropic/…` → `claude:`), or reported unavailable with the reason; never swapped. The renderer's `AiModelOptionsService` + `<app-ai-model-select>` is every picker (Process inspector, Add dialog, queue item, Settings › AI Analysis, the setup wizard's AI step), bound with ngModel, refreshed on `crucible.readiness`, `crucible.servers-changed` and a server's coordination reaching `stocked`. A choice is queued and saved in its option spelling; saved defaults are rewritten only when the user saves them. The scorer's decide model is not user-selectable (P6). Transcription's asr picker is Settings › Transcription's, from the server's `/v1/info` asr rows.
