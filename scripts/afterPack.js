 ...
 * are copied via a filesystem or tool that can't store them inline (common when
 * the project lives on exFAT). If any land inside the signed bundle they can
 * fail Apple's notary check ("resource fork, Finder information, or similar
 * detritus not allowed"). Runs in afterPack, before codesign, so the sealed
 * bundle is always clean regardless of the source tree's state.
 */
function stripAppleDoubleFiles(context) {
  const appName = context.packager.appInfo.productFilename;
