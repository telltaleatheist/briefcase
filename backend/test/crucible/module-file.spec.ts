...
describe('briefcase.module.json', () => {
  ...
  it('asks for llm and asr, the analysis class, and large-v3 whisper on both backends', () => {
    expect(BRIEFCASE_MODULE.job_types.map((j) => j.type)).toEqual(['llm', 'asr']);
    expect(BRIEFCASE_MODULE.needs).toEqual([{ class: 'analysis' }]);
    expect(BRIEFCASE_MODULE.subjects.map((s) => s.id)).toEqual(['faster-whisper-large-v3', 'mlx-whisper-large-v3']);
  });

  it('is filtered to one backend and stripped of `backends` before posting', () => {
    const mac = moduleForBackend('mlx-darwin');
    expect(mac.subjects).toEqual([{ kind: 'model', id: 'mlx-whisper-large-v3' }]);
    expect(mac.job_types).toEqual([{ type: 'llm' }, { type: 'asr' }]);
    expect(JSON.stringify(mac)).not.toContain('backends');

    const pc = moduleForBackend('cuda-linux');
    expect(pc.subjects).toEqual([{ kind: 'model', id: 'faster-whisper-large-v3' }]);
