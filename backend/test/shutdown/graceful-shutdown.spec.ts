 ...
 * among them) once, and a hard exit under Electron's 12 s kill.
 */
...
describe('REGRESSION: the quit path runs every shutdown hook exactly once', () => {
  it('SIGTERM twice and a SIGINT: one close, one sweep, one exit', async () => {
    ...
    const exit = jest.fn();
    const handler = installGracefulShutdown({ close: () => app.close(), exit, log: quiet }, ['SIGTERM', 'SIGINT'], proc as never);
  ...
  it('a hook that hangs cannot keep the process past the deadline, which is under Electron\'s 12 s kill', async () => {
    ...
    const exit = jest.fn();
    void gracefulShutdown({ close: () => new Promise(() => undefined), exit, log: quiet, deadlineMs: 50 })('SIGTERM');
