...
import { SentenceUnit, assembleUnits, unitsFromSentences } from './units';

/** Times and text only (the shape chapters use). */
const tt = (units: SentenceUnit[]) => units.map(({ start, end, text }) => ({ start, end, text }));

describe('assembleUnits', () => {
  it('keeps measured segment times across sentence splits', () => {
    const units = tt(assembleUnits([
  ...
  it('does not split a long sentence inside one segment', () => {
    ...
    expect(tt(assembleUnits([{ start: 0, end: 40, text }]))).toEqual([{ start: 0, end: 40, text }]);
  });
  ...
  it('cuts a run-on sentence only where segments overlap it, not the neighbouring sentences', () => {
    ...
    const units = tt(all);
    // Sentence ranges: intro = 0, the run-on pieces all lie in sentence 1, the tail is 2.
    expect(all.map((u) => [u.sentenceFrom, u.sentenceTo])).toEqual([[0, 0], ...all.slice(1, -1).map(() => [1, 1]), [2, 2]]);
  ...
  it('records the inclusive sentence range of every unit, through the fold', () => {
    ...
    expect(units.map((u) => [u.index, u.sentenceFrom, u.sentenceTo])).toEqual([
      [0, 0, 2],
      [1, 3, 3],
    ]);
  });

  it('from sentences alone: the same fold, and a run-on cut by words keeps its sentence times', () => {
    const sentences = [
      { start: 0, end: 1, text: 'Okay.' },
      { start: 1, end: 5, text: 'So here is the actual point.' },
      { start: 10, end: 70, text: Array.from({ length: 90 }, (_, i) => `w${i}`).join(' ') },
    ];
    const units = unitsFromSentences(sentences);
